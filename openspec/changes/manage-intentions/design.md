...
`IntentionScope` различает активные, архивированные и все существующие намерения. `IntentionCatalogOrder` состоит из выбранного поля — `createdAt` или `updatedAt` — и направления `ascending` или `descending`; начальное значение равно `createdAt descending`. `IntentionCatalogQuery` содержит scope, нормализуемый фильтр названия, порядок, проверяемый конечный размер порции и необязательный `IntentionCatalogCursor`, полученный из предыдущей страницы того же запроса. Cursor остаётся storage-neutral value object: caller только возвращает его repository и не строит из SQLite row, offset или SQL-выражения. При изменении scope, фильтра или порядка прежний cursor отбрасывается.

`IntentionCatalogPage` всегда содержит не больше запрошенного ограниченного размера, точное количество всех совпадений до разбиения на порции и cursor следующей порции либо `null` после конца выдачи. `IntentionSummary` несёт только необходимые каталогу идентификатор, название, наличие описания, готовность, архивное состояние и timestamps; полный текст описания получает только `watchById`. Это ограничивает I/O и память независимо от размера описаний. Невалидный или не соответствующий запросу cursor и недопустимый размер порции возвращают стабильную validation failure, а не выполняют неограниченное чтение.
...
- фильтр каталога перед запросом проходит `trim()` и регистронезависимое Unicode-преобразование, совпадающее с ключом поискового индекса; пустой результат `trim()` означает отсутствие фильтра, внутренние пробелы и различие `е`/`ё` и диакритики сохраняются;
- cursor строится из выбранной временной метки и `IntentionId`, которые вместе задают полный порядок; идентификатор является только автоматическим tie-breaker и не становится пользовательской настройкой;
...
Непустой фильтр длиной не меньше трёх Unicode-символов преобразуется в безопасно экранированную единственную FTS phrase и передаётся параметром `MATCH`; это не позволяет пользовательскому тексту стать FTS operator. Поскольку trigram не индексирует подстроки короче трёх символов, такие фильтры используют параметризованный `instr(title_search_key, query_search_key)` по основной таблице. Оба пути дополнительно проверяются одинаковыми fixtures, чтобы наблюдаемая семантика буквальной подстроки не зависела от выбранной ветви.
...
- repository integration tests проверяют ограничение размера страницы, точный count, cursor boundary при равных timestamps, отсутствие offset, буквальную регистронезависимую фильтрацию на русском и английском, отдельную ветвь для 1–2 символов, различие `е`/`ё` и диакритики, атомарную согласованность FTS triggers, streams подробных данных после commit, закрытую/повреждённую базу и преобразование storage exceptions в failures;
...
- **[Cursor может вернуть пропуски или повторы при неполном порядке либо смене параметров]** → Включать timestamp и `IntentionId` в полный порядок, связывать cursor с нормализованными параметрами запроса, отбрасывать его при их изменении и проверять границы с равными timestamps.
...
Действующих ADR, требующих пересмотра или supersession, нет. Ограниченная страничная операция уточняет чтение через прежнюю seam `IntentionRepository`, а FTS5 остаётся обратимо мигрируемым индексом внутри уже принятого Drift/SQLite adapter; границы глубокого модуля, технология хранения, идентичность и политика локальной приватности из ADR-0001–ADR-0004 не меняются.
