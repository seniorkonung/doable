...
- **[FTS-индекс может разойтись с основной таблицей после ошибки записи, миграции или перезаписи hidden rowids]** → Обслуживать external-content index транзакционными triggers, запретить `VACUUM` и иные нетранзакционные rowid-rewriting paths, сохранять rowids либо атомарно перестраивать FTS при table rebuild и до commit выполнять index-aware `integrity-check` с `rank = 1`; отрицательный test доказывает, что проверка падает на намеренно рассогласованном индексе.
...
4. Явно связать Android production connection `root/app_flutter/doable.sqlite` с исключением всего `root/app_flutter/` в обоих наборах backup rules и их contract test, затем создать `AppDatabase` schema version 1 с UUID v7, временными метками намерения, внутренним поисковым ключом, FTS5 trigram-индексом, его triggers и index-aware `integrity-check` с отрицательной fixture, initial schema snapshot, атомарным migration harness, проверкой несовместимой более новой версии и in-memory executor.
