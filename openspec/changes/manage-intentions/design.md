...
- repository получает функцию текущего UTC wall-clock time через constructor injection, чтобы создание, обновление и граничные случаи одинаковых временных меток были детерминированно проверяемы без отдельного глобального clock service; значение `updatedAt` не обязано строго возрастать относительно предыдущего значения, а его равенство или уменьшение после коррекции системных часов считается принятым остаточным риском при сохранении предметного инварианта `updatedAt >= createdAt`;
...
- **[Новая фактически временная причина может отсутствовать в retryable allowlist и получить unexpected outcome]** → В bootstrap и repository считать retryable только доказанные машинными кодами причины, проверять классификацию на in-process и production-подобном background executor и расширять allowlist отдельным подтверждённым решением вместо catch-all retry.
