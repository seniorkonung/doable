...
- Экранная ViewModel реализуется class-based Riverpod Notifier, формирует неизменяемое UI state и передаёт типизированные `IntentionCommand` feature-specific `IntentionCommandCoordinator`; после принятия command экран только наблюдает её outcome, пока остаётся открытым.
...
- View реагирует через `ref.listen` на одноразовый UI event навигацией, SnackBar или доступным сообщением и после обработки вызывает метод ViewModel для очистки event. Catalog ViewModel сохраняет уведомление о completion, полученном пока дочерний route находится сверху, до возвращения каталога в активное представление.
