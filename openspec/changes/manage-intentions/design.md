...
- предметная модель хранит неизменяемый `createdAt` и изменяемый `updatedAt` как независимые UTC-показания системных часов; при создании они равны, а при фактическом успешном изменении `updatedAt` заменяется текущим показанием без требования быть больше прежнего значения или `createdAt`;
