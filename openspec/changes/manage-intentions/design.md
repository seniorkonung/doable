...
Согласно ADR-0002 `QueryExecutor` остаётся внутренней низкоуровневой подстановкой модуля соединений; произвольный executor не является допустимым входом обычного object graph. Закрытый platform-neutral тип `ConfiguredLocalDatabaseConnection` владеет executor и служит статическим доказательством того, что обязательный connection setup уже применён. Только factories local connection module могут создать это значение; `AppDatabase` принимает capability, а `LocalDataBootstrap` — фабрику таких значений вместо фабрики raw `QueryExecutor`. Android production factory явно передаёт `getApplicationDocumentsDirectory` как `DriftNativeOptions.databaseDirectory` в `driftDatabase(name: 'doable', ...)`, получает файл `doable.sqlite` и выполняет SQLite в background isolate, а in-memory/file-backed, migration и schema-verification factories выбирают собственный native executor за той же типизированной границей.

Единая connection setup регистрирует `doable_title_search_key` на каждом underlying SQLite connection до разбора и использования schema, включая production background isolate, in-memory/file-backed harness, migration connections и обе стороны полной schema verification. Дополнительный fixture setup выполняется до канонической регистрации: последняя регистрация зарезервированного имени и arity всегда принадлежит Doable, поэтому обычная композиция не может неявно затенить schema-function. Decorator или fault interceptor принимает и возвращает `ConfiguredLocalDatabaseConnection`, оборачивает уже выбранный executor без повторного открытия и не ослабляет доказательство setup. Missing-setup и намеренный mapping drift проверяются только низкоуровневыми явно названными test fixtures вне обычного `AppDatabase`/bootstrap object graph; общий production-доступ к небезопасному constructor или произвольной замене зарезервированной функции не вводится. Универсальный registry либо DSL для schema-functions не добавляется, пока второй реальный function/collation contract не докажет необходимость такого обобщения.

Компонентная граница внутри Flutter application container выглядит так:

```text
+------------------------+   +-------------------------+   +----------------------+
| Android production     |   | In-memory / file-backed |   | Migration / verifier |
| connection factory     |   | connection factories    |   | connection factories |
+-----------+------------+   +------------+------------+   +----------+-----------+
            \                         |                             /
             \                        v                            /
              +--------------------------------------------------+
              | fixture extension -> canonical Doable setup      |
              +-------------------------+------------------------+
                                        |
                                        v
                         +-----------------------------------+
                         ...
                         +-----------------+-----------------+
                                         |
                          typed decorator|fault interceptor
                                         v
                              +---------------------+
                              | AppDatabase / Drift |
                              +----------+----------+
...
Raw missing-setup и alternate-mapping fixtures остаются отдельной test-only
ветвью и не создают ConfiguredLocalDatabaseConnection.
```

- Factories владеют выбором платформы и физического executor; `AppDatabase` не получает Android paths или platform APIs.
- Opaque connection capability переносит доказательство обязательного setup через bootstrap, migrations, verifier и test harnesses.
- Typed decorators сохраняют гарантию, тогда как обычная fixture-композиция не может переопределить зарезервированную schema-function.
- Негативные и mapping-drift сценарии намеренно работают ниже защищённой границы и потому не превращаются в альтернативный production constructor.
...
- connection-contract tests проводят Android production-like background, in-memory/file-backed, migration и обе schema-verification стороны через `ConfiguredLocalDatabaseConnection`, подтверждают одинаковую прямую SQL-семантику зарезервированной функции и сохранение capability после fault interception. Отдельная fixture пытается зарегистрировать то же имя и arity через обычную композицию и доказывает, что каноническая Doable-регистрация остаётся последней; raw test-only fixtures отдельно доказывают controlled failure зависимой схемы без setup и допустимый mapping drift двух сборок, не предоставляя обходной constructor для `AppDatabase`;
...
- **[Application-defined search-key function вызывается из schema, а новый connection path или fixture может пропустить либо затенить её setup]** → Разрешить schema-вызов только для total pure function одного `TEXT`, которая не выполняет I/O, не читает внешнее состояние и не раскрывает данные; открывать только внутренний app-specific SQLite-файл без импорта. Передавать в `AppDatabase`, bootstrap, migrations и verifier только непрозрачную `ConfiguredLocalDatabaseConnection`, создаваемую общими factories после канонической последней регистрации; сохранять capability при декорировании, а missing-setup и alternate mapping оставлять в явно отделённых raw test fixtures. При появлении импорта чужих баз, `trusted_schema = OFF` или более широкой модели угроз сначала перейти на innocuous-capable registration либо убрать application-defined function из schema.
