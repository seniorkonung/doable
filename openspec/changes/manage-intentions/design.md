...
Doable остаётся Flutter-приложением с Android host без постоянного пользовательского хранилища. В рамках начатого change уже завершены предметный и прикладной контракты Phase 2 и типобезопасная граница идентичности Phase 3: `IntentionId` представляет канонический ненулевой UUID независимо от версии, новые значения создаются внедряемым UUIDv7-generator, а сохранённые представления восстанавливаются через типизированное декодирование; Drift/SQLite adapter ещё не реализован. Зафиксированный стек — Flutter 3.47.1 stable и Dart 3.13.1. Design ограничивают действующие ADR-0002 о Drift/SQLite, ADR-0003 о типобезопасной UUID-идентичности со сменной политикой генерации, ADR-0004 о локальном Android storage, ADR-0005 о глубоком модуле с ограниченными снимками каталога, ADR-0007 о корректном Unicode без NUL и ADR-0008 о долговечном контракте SQLite schema-functions. ADR-0005 supersedes ADR-0001, который остаётся историческим контекстом и больше не задаёт действующий repository contract.

Все ADR-0001–ADR-0008 принадлежат активному `manage-intentions` и имеют статус `proposed`. Здесь «действующий ADR» означает выбранное решение внутри этого change; его можно редактировать напрямую до архивации. Только архивация переводит статус в `accepted` и делает документ неизменяемым. Манифест `adr.md` фиксирует принадлежность документов и отличает завершение проверки от окончательного принятия ADR.
...
- не позволять `U+0000` или malformed UTF-16 пересечь границу пользовательского текста и попасть в нормализацию, кодирование, хранение либо поиск, сохраняя остальные корректные скалярные значения Unicode без дополнительной нормализации;
...
- сохранять уже загруженный каталог и позицию прокрутки при внутренних переходах, отражая подтверждённые изменения через типизированные terminal outcomes независимо от lifetime дочернего экрана и однозначно согласуя их с конкурентными snapshot-чтениями;
...
```text
Container: Flutter application

+----------------------------------------------------------------------------+
| Composition root: AppRuntime + bootstrap + ProviderContainer + router      |
|                                                                            |
|  +----------------------+   command   +-------------------------------+    |
|  | Route ViewModels     | ----------> | IntentionCommandCoordinator   |    |
|  | detail generation    | <---------- | operations + completions      |    |
|  +----------------------+             +--------+---------------+------+    |
|                                               | completion    | execute     |
|                                               v               v             |
|                              +----------------------+  +------------------+ |
|                              | Catalog ViewModel    |  | Repository       | |
|                              | bounded snapshot     |  | rules + Result   | |
|                              +----------+-----------+  +---------+--------+ |
|                                         | query                  |          |
|                                         +----------------------->|          |
|                                                                 v          |
...
- View знает только generated provider собственной ViewModel; widgets не обращаются к coordinator, repository или Drift.
- Экранная ViewModel реализуется class-based Riverpod Notifier, формирует неизменяемое UI state и передаёт типизированные `IntentionCommand` feature-specific `IntentionCommandCoordinator`; после принятия command экран наблюдает её outcome, пока остаётся открытым, и владеет приоритетным presentation claim для operation token этой попытки.
- `IntentionCommandCoordinator` является глубоким application module за generated keep-alive Riverpod provider: он скрывает lifetime принятых операций, сериализацию по `IntentionId` и публикацию terminal outcomes, но не хранит формы, навигацию, локализацию или снимок каталога. Отдельные публичные port и adapter для него не вводятся.
...
- Drift остаётся implementation detail repository. Тесты repository получают `ConfiguredLocalDatabaseConnection` через общие in-memory/file-backed factories согласно ADR-0002; низкоуровневый `QueryExecutor` скрыт внутри этой границы. Тесты ViewModel используют `ProviderContainer` с override на fake adapter `IntentionRepository`.
- Единственный process-local `AppRuntime` владеет `LocalDataBootstrap` и вручную созданным `ProviderContainer`, а UI получает уже созданный container через `UncontrolledProviderScope`. Generated providers по-прежнему связывают repository, coordinator, router и presentation dependencies, но `ref.onDispose` освобождает только синхронные provider-local ресурсы и не считается владельцем асинхронного закрытия базы. `AppRuntime` не является глобальным singleton или service locator и остаётся единственной точкой управляемого создания и shutdown object graph.
...
`IntentionCatalogPage` является sealed-контрактом и всегда содержит не больше запрошенного ограниченного размера, cursor следующей порции либо `null` после конца выдачи и непрозрачную `IntentionCatalogRevision`. Запрос без cursor возвращает только `IntentionCatalogFirstPage` с точным количеством всех совпадений до разбиения на порции; её release-safe constructor не допускает отрицательный count или count меньше числа возвращённых summaries. Запрос с cursor возвращает только `IntentionCatalogContinuationPage` без повторного count. Поэтому отсутствие count у продолжения не представляется nullable-полем, а caller исчерпывающе различает начальный snapshot и его продолжение. `IntentionSummary` несёт только необходимые каталогу идентификатор, название, наличие описания, готовность, архивное состояние и timestamps; полный текст описания публично получает только `watchById`. Для каждой из не более чем `pageSize + 1` прочитанных строк закрытая storage-mapping boundary получает исходное nullable-описание, проверяет его полную предметную семантику и отбрасывает текст до построения summary; тем самым недопустимая строка не превращается в ложный `hasDescription`, а внутренние I/O и память остаются ограничены размером порции и максимальной допустимой длиной описания. Предметная модель и summary принимают каждое представимое UTC-значение независимо от взаимного порядка `createdAt` и `updatedAt`: обратный порядок после перевода системных часов не означает повреждение данных. Невалидный или не соответствующий запросу cursor, размер порции вне диапазона 1–100, строка фильтра вне общего Unicode-repertoire либо непустой нормализованный фильтр длиннее 255 расширенных графемных кластеров возвращают стабильную validation failure до обращения к storage adapter, а не выполняют чтение каталога.

Закрытый набор `IntentionCommand` содержит создание, изменение данных, включение или выключение готовности к действию, архивирование, восстановление и физическое удаление. Наличие одной `execute` не стирает различия операций: варианты command имеют собственные обязательные параметры, а `Result` возвращает типизированный успех или предметную/инфраструктурную failure. Каждый success несёт sealed-вариант `IntentionCatalogMutation` с точной ревизией подтверждённого состояния и непрозрачными снимками каталожной записи до и после выполнения: `IntentionCatalogCreated` не имеет `before`, `IntentionCatalogDeleted` — `after`, `IntentionCatalogUpdated` требует оба снимка, а `IntentionCatalogUnchanged` возвращает один и тот же снимок с обеих сторон. Поэтому комбинация без `before` и `after` не представима. `IntentionCatalogEntrySnapshot.matches` отвечает только за scope/filter текущего query и намеренно игнорирует его cursor и page size; порядок определяется публичным `summary`. Реализация снимка сохраняет точный результат принадлежности согласно фактически сохранённой до/после выполнения поисковой проекции и не раскрывает её сырой ключ, SQLite или FTS. Это существенно при допустимом дрейфе Unicode-данных между сборками: Catalog ViewModel не пытается восстановить историческое совпадение повторным вычислением из одного `title`. Создание и любая сохраняющая command возвращают `IntentionSaved` с подтверждённым после commit намерением. No-op также возвращает текущий `IntentionSaved` с `IntentionCatalogUnchanged`, не выполняет запись, не меняет timestamps и не продвигает ревизию. Успешное физическое удаление возвращает `IntentionDeleted` с удалённым `IntentionId` и mutation только после commit; отсутствие идентификатора остаётся typed not-found failure.

`getCatalogPage` возвращает один ограниченный подтверждённый snapshot через `Future<Result<...>>`; каталог не является stream всех намерений. Ошибка чтения возвращается как типизированная repository failure, а явный retry повторяет только требуемую страницу при доказанно устранимой недоступности. `watchById` продолжает публиковать `ResultSuccess` с подтверждённым snapshot или успешным отсутствием намерения; adapter преобразует ошибку в один `ResultFailure`, после чего текущий stream завершается, а retryable failure допускает новую подписку. Таким образом, все failure проходят одну исчерпывающую Result-модель, а нетипизированный Dart error channel и Drift/SQLite exceptions никогда не пересекают seam. `watchById` намеренно не получает `IntentionCatalogRevision`: порядок между его результатами и успешной локальной командой согласует Details ViewModel внутри presentation layer. После terminal outcome command coordinator единожды публикует data completion уже открытому Catalog ViewModel и делает ту же публикацию доступной каждому смонтированному Details ViewModel независимо от lifetime инициировавшего экрана и владельца пользовательского сообщения. Catalog success применяет точный `IntentionCatalogMutation`; Details ViewModel отбирает только успешный `IntentionSaved` или `IntentionDeleted`, чей payload содержит его `IntentionId`, а failure completion не изменяет подробные данные. `watchById` после удаления по-прежнему публикует `ResultSuccess(null)`.

`IntentionRepository` является storage-neutral seam: его interface не раскрывает расположение данных, transport, состояние синхронизации или типы конкретного adapter. В текущем change production adapter использует только Drift, а fake adapter заменяет его в тестах ViewModels. Будущий adapter сможет внутренне скомпоновать локальный и удалённый источники, не заставляя Flutter Views зависеть от их протокола. Отдельные `SyncRepository`, remote port, outbox, tombstones, долговечные или синхронизационные revisions и конфликтные версии сейчас не вводятся: без наблюдаемого sync-контракта они образовали бы спекулятивные shallow modules и зафиксировали бы случайную семантику. Вводимая `IntentionCatalogRevision` служит только process-local согласованию чтений и записей одного живого repository и не переживает пересоздание object graph. В соответствии с [ADR-0006](../../../docs/adr/0006-treat-device-clock-timestamps-as-observations.md) `updatedAt` остаётся показанием изменяемых UTC wall-clock часов клиентского устройства при последнем подтверждённом изменении и не является revision, causal clock, гарантией фактической хронологии или правилом разрешения конфликтов.

Каждый экземпляр `IntentionRepository` владеет уникальной эфемерной эпохой, согласованной с owner token его cursors, и монотонным целым номером mutation в памяти. Закрытый асинхронный sequencer пропускает через один порядок полное snapshot-чтение каждой страницы и commit каждой изменяющей command. Страница получает текущую пару `(epoch, sequence)` после завершения согласованного чтения и до освобождения sequencer. Успешная изменяющая транзакция после commit, но до освобождения sequencer, продвигает sequence ровно на единицу и возвращает эту ревизию вместе с точными `before`/`after`; failure и no-op sequence не меняют. Сама доставка уже сформированного `Future`-результата caller может задержаться, поэтому ревизия нужна даже при последовательном выполнении repository operations.

```text
+------------+     +--------------+     +-------------+     +----------------------+     +--------+
| Catalog VM |     | Future queue |     | Coordinator |     | Repository sequencer |     | SQLite |
+------------+     +--------------+     +-------------+     +----------------------+     +--------+
      | page request R10                 |                         |                         |
      |---------------------------------------------------------->| read snapshot           |
      |                            |     |                         |------------------------>|
      ...
      | apply mutation R11         |     |                          |                         |
      |<-- delayed page R10 -------|     |                          |                         |
      | discard and retry only this page |                          |                         |
```

Первая страница новой query generation задаёт авторитетную основу на своей ревизии. Completions, поступившие во время её получения, временно удерживаются: mutation с более новой ревизией применяется к основе по порядку, а уже включённая в ревизию страницы повторно не применяется. Для существующего каталога страница старше уже применённой ревизии отбрасывается и повторно запрашивается с тем же cursor без чтения предшествующих порций, изменения сохранённой boundary, count или visual anchor. Страница той же ревизии объединяется сразу. Страница новее текущего состояния удерживается как единственная pending page, пока упорядоченные completions не продвинут каталог до её ревизии, после чего объединяется. Ревизии разных эпох несравнимы: новый Catalog ViewModel после пересоздания repository начинает новую query с первой страницы, а старые cursors и in-flight результаты не переносятся.

Этот протокол предполагает, что все production-записи текущего change проходят через один экземпляр repository/coordinator. Записи из другого процесса, isolate, внешнего sync adapter или импортированной базы потребуют отдельного change с долговечной либо предоставляемой источником ревизией. В текущей границе Catalog ViewModel хранит только применённую ревизию, не более одной pending page и конечное число completions выполняющихся операций; вечный набор обработанных operation tokens не нужен.

Модуль владеет следующими правилами:

- до `trim()`, обхода `runes`, подсчёта `characters`, case folding и любого UTF-8/SQLite-перехода одна чистая storage-neutral операция проверяет UTF-16 code units исходной Dart-строки, отклоняет непарные surrogates и `U+0000` и возвращает доверенное корректное Unicode-представление либо типизированную причину с затронутым полем; она не удаляет и не заменяет символы, не выполняет Unicode-нормализацию и не запрещает остальные корректные управляющие или форматирующие скалярные значения;
- название, описание и фильтр сохраняют отдельные типы и field-specific правила поверх общей проверки; универсальный публичный `UserText` не стирает различия их семантики, а ViewModel локализует стабильную причину и сохраняет исходное значение для исправления;
...
- первая страница и её `totalCount` читаются в одной read transaction из одного SQLite snapshot; продолжения того же запроса не выполняют повторный count, а Catalog ViewModel сохраняет начальное точное значение и корректирует его только по membership transition подтверждённых command completions;
- page reads и commits изменяющих commands проходят через один process-local sequencer repository; каждая страница получает ревизию своего snapshot, а каждый фактически изменивший состояние success — следующую ревизию и точные непрозрачные снимки каталожной записи до/после commit;
...
- Приложение получает один вручную созданный `ProviderContainer` через корневой `UncontrolledProviderScope`. Process-local `AppRuntime` создаёт и удерживает `LocalDataBootstrap`, настраивает container с отключённым automatic retry и предоставляет принадлежащие runtime значения generated `keepAlive` providers; подтверждённый database, repository, `IntentionCommandCoordinator` и router остаются зависимостями provider graph. `ref.onDispose` не запускает fire-and-forget закрытие `AppDatabase`: ожидаемым асинхронным shutdown владеет только `AppRuntime`.
...
- `watchById` оформляется закрытым generated functional Stream provider, параметризованным `IntentionId` и приватным монотонным `DetailObservationGeneration`. Details ViewModel синхронно устанавливает listener data completions coordinator до запуска первой read generation и остаётся единственным владельцем эффективного неизменяемого состояния подробного просмотра; View не объединяет stream и command result самостоятельно. Каждый обработчик результата `watchById` захватывает generation своей подписки и может обновить состояние только пока эта generation является текущей; ожидание первого результата показывает `loading` лишь при отсутствии уже подтверждённого намерения, поэтому запуск post-success generation сам по себе не скрывает `IntentionSaved`. Catalog ViewModel отдельно различает initial loading/error/empty и сохраняет подтверждённые `items`, `totalCount`, `nextCursor`. Состояние продолжения является закрытым набором взаимоисключающих вариантов: покой, загрузка, retryable `unavailable`, terminal `corruption`, terminal `unexpected`, ожидающий явного восстановления `validation` и восстановление с первой страницы. Только вариант `unavailable` предоставляет retry той же порции, только `validation` — отдельное восстановление без прежнего cursor, а terminal-варианты не могут получить обычный retry. Ни один failure продолжения не заменяет уже загруженные данные.
- Создание и изменения используют общий in-process presentation primitive `ExclusiveOperation` с одной синхронной операцией `start`. Первый вызов резервирует экземпляр до запуска асинхронного действия и возвращает принятое выполнение с его `Future`; повторный вызов занятого экземпляра возвращает отдельный outcome `alreadyRunning`, не вызывает действие и не ставит его в очередь. Gate освобождается после любого завершения принятого `Future`, включая failure или неожиданную ошибку. Неизменяемый `OperationState<TResult>` со статусами `idle`, `running`, `succeeded` и `failed` остаётся частью UI state экранной ViewModel: она синхронно публикует `running` и преобразует завершившийся outcome в итоговое состояние, только пока provider смонтирован.
- Generated keep-alive `IntentionCommandCoordinator` принимает command до первого asynchronous gap и становится единственным владельцем её repository `Future` до terminal outcome. Для commands существующего намерения coordinator хранит только выполняющийся `ExclusiveOperation` по `IntentionId`; поэтому уход и повторное открытие details не освобождают gate, все изменяющие controls того же намерения остаются недоступны, а другой `IntentionId` работает независимо. Editor ViewModel сохраняет отдельный form-local gate против повторной отправки, но после принятия создание также выполняется coordinator и не зависит от lifetime формы.
- Каждый принятый запуск получает новый непрозрачный process-local operation token для связи UI с конкретным выполнением. Coordinator публикует один непереигрываемый `IntentionCommandCompletion` в порядке repository mutation revisions: безопасный вид операции и `Result<IntentionCommandSuccess>` без route, BuildContext или локализованной строки. Одна публикация доступна Catalog ViewModel и всем уже смонтированным Details ViewModels; каждый Details ViewModel отбирает только успешный payload с собственным `IntentionId` и игнорирует failures при согласовании данных. Количество data consumers не меняет terminal outcome и не потребляет presentation claim. Доставка completion для согласования данных всегда отделена от presentation ownership.
- Для каждого token coordinator предоставляет единственный типизированный presentation claim с вариантами `initiator` и `catalogFallback`. Инициировавшая ViewModel удерживает приоритетный claim только до disposal своей экранной сессии: при terminal outcome она синхронно публикует terminal state или одноразовый event в остающийся смонтированным UI state и подтверждает потребление; при disposal освобождает непотреблённый claim. Catalog ViewModel может получить только освобождённый fallback claim. Claim, его подтверждение и освобождение сериализованы coordinator, поэтому один outcome нельзя одновременно назначить обоим владельцам или потерять в гонке terminal outcome с уходом. После публикации data completion и разрешения presentation claim terminal entry удаляется; coordinator не сохраняет пользовательский текст, завершённую историю или replay-журнал.
- Подписка Catalog ViewModel существует вместе с уже открытым каталогом. Каждый success согласуется по revision и точным `before`/`after` независимо от presentation owner, а failure сохраняет snapshot. Каталог создаёт безопасное одноразовое сообщение только для completion, чей `catalogFallback` claim он получил; outcome с подтверждённым claim инициирующего экрана не становится ожидающим сообщением каталога. Новый Catalog ViewModel без прежней подписки начинает с repository snapshot и не требует replay process-local completions.
- Уход с editor или details всегда доступен и не вызывает cancellation принятого command. Если экран остаётся открытым, его ViewModel наблюдает тот же accepted `Future`, показывает terminal state и потребляет presentation claim; явный retry получает новый token и не возвращает уже потреблённый failure в каталог. После disposal экранной ViewModel coordinator завершает работу без обращения к уничтоженному `Ref`, а непотреблённый outcome передаёт catalog fallback. Повторное открытие details читает running state coordinator по `IntentionId`.
- Details ViewModel согласует подтверждённые данные команды только через успешный `IntentionCommandCompletion`, чей `IntentionSaved` или `IntentionDeleted` содержит его `IntentionId`, а не повторно через результат наблюдаемого accepted `Future`; сам Future управляет состоянием операции и presentation claim инициировавшей экранной сессии. При каждом таком `IntentionSaved`, включая no-op, ViewModel до любого asynchronous gap продвигает `DetailObservationGeneration`, публикует возвращённое после commit намерение как авторитетное подтверждённое состояние и создаёт новую `watchById`-подписку. До первого результата новой generation это состояние остаётся видимым, а обработчики всех прежних generations игнорируются даже тогда, когда отменённая подписка уже поставила событие в очередь; cancellation остаётся освобождением ресурса, а не доказательством порядка. Failure completion не является data path подробного состояния, не продвигает read generation и не заменяет прежний подтверждённый snapshot. При `IntentionDeleted` ViewModel сначала продвигает generation и прекращает текущую подписку, затем публикует одноразовое событие завершения route; ни один прежний non-null результат больше не может показать удалённое намерение. Если details открывается уже после terminal outcome, его listener устанавливается до новой post-commit read generation, поэтому replay completion history не требуется.
- Управляемый shutdown всего object graph является отдельной двухфазной границей. `AppRuntime` синхронно переходит в состояние closing и переводит coordinator в draining до уничтожения provider graph: новые commands больше не принимаются, UI consumers и subscriptions прекращаются при `ProviderContainer.dispose()`, а сохранённая runtime ссылка на coordinator дожидается всех уже принятых repository `Future` без обращения к уничтоженному `Ref` и без ожидания presentation claims исчезнувшего UI. После drain runtime ровно один раз ожидает `LocalDataBootstrap.close()` и только затем считается closed; новый runtime над тем же SQLite-файлом можно создавать лишь после завершения этого `Future`. Внезапное завершение Android-процесса не является управляемым shutdown и не обещает ожидание Future: целостность в этой границе по-прежнему обеспечивают SQLite-транзакция и следующий bootstrap. Экспериментальные Riverpod Mutations, offline persistence, durable event log и sync abstraction не используются.
...
- View реагирует через `ref.listen` на одноразовый UI event навигацией, SnackBar или доступным сообщением и после обработки вызывает метод ViewModel для очистки event. Инициирующий экран публикует event только после получения своего presentation claim. Catalog ViewModel сохраняет до возвращения в активное представление только уведомление с полученным `catalogFallback` claim; уже потреблённый дочерним экраном outcome не дублируется, а failure прежней попытки не переживает успешный retry с новым token.
...
- После debounce Catalog ViewModel получает `IntentionCatalogFirstPage`, принимает из неё точный count, cursor и revision, а завершившиеся конкурентные commands согласует с этой основой в порядке ревизий. `ScrollController` инициирует `loadNextPage`, когда до конца остаётся не больше `prefetchRemaining` элементов; synchronous single-flight guard не допускает второй запрос той же страницы. Успешная `IntentionCatalogContinuationPage` той же применённой ревизии добавляет только новые summaries по `IntentionId`, сохраняет прежний точный count и обновляет cursor; более старая страница отбрасывается и повторяется с тем же cursor, более новая кратко удерживается до предшествующих completions. `nextCursor == null` прекращает подгрузку. `IntentionUnavailableFailure` сохраняет прежние items, count и cursor boundary и предоставляет retry только той же порции; `IntentionCorruptionFailure` и `IntentionUnexpectedFailure` сохраняют их в terminal inline-состоянии без обычного retry. `IntentionValidationFailure` также сохраняет подтверждённый префикс, но предоставляет отдельное явное восстановление: ViewModel увеличивает query generation и запрашивает первую страницу тех же scope/filter/order с `cursor: null`, не очищая видимые данные на время запроса и не принимая ответы прежней generation. Успешная первая страница атомарно заменяет накопленные items, count, cursor и revision; failure сохраняет прежний префикс и представляется по правилам соответствующего initial-page результата, поэтому прежний недопустимый cursor не повторяется и validation не создаёт автоматический цикл. Обычное изменение scope, фильтра или порядка создаёт новую query generation без cursor, очищает прежние порции, не принимает результаты прежней generation и получает новый count вместе с её первой страницей.
- Накопленные summaries всегда образуют непрерывный префикс актуального глобального порядка до сохранённой cursor boundary, но его длина может измениться после command. При `nextCursor == null` весь результат уже загружен, поэтому совпадающий `after` вставляется или перемещается в любое правильное место. При наличии следующей порции Catalog ViewModel использует точные membership-состояния `before`/`after` для текущего query и storage-neutral order его summaries: `after` с позицией не позже boundary вставляется или перемещается внутри префикса, а `after` после boundary либо вне scope/filter удаляется или не добавляется; отсутствие `after` означает удаление. Переход `before.matches(query)` → `after.matches(query)` корректирует точный count ровно один раз. Сохранённый cursor не меняется, следующая порция запрашивается от прежней value boundary и дедуплицируется по `IntentionId`. Предшествующие порции повторно не читаются, результат не сбрасывается на первую страницу, а mutation, уже включённая в revision страницы, второй раз не применяется.
- Каталог строится через `ListView.builder`, поэтому Flutter [создаёт widgets по мере попадания в viewport](https://docs.flutter.dev/resources/inside-flutter#infinite-scrolling), а не для всех накопленных summaries. [`ScrollController.keepScrollOffset`](https://api.flutter.dev/flutter/widgets/ScrollController/keepScrollOffset.html) и уникальный [`PageStorageKey`](https://api.flutter.dev/flutter/widgets/PageStorage-class.html) сохраняют position внутри route; пока подробный экран находится поверх каталога, его ViewModel и накопленные страницы остаются тем же экранным состоянием. Перед локальным согласованием command completion ViewModel сохраняет `IntentionId` первого видимого summary и внутристочный offset, после согласования восстанавливает этот visual anchor, если summary осталось, иначе использует ближайшего оставшегося соседа. Успешная операция сообщает о сохранении независимо от того, осталось ли изменённое намерение в загруженном префиксе.
...
- Riverpod 3 по умолчанию автоматически повторяет упавшие providers. Владеющий `ProviderContainer` задаёт `retry: (retryCount, error) => null`, чтобы loading/failure и повторная попытка оставались явным пользовательским поведением; retry выполняется только через целевую invalidation соответствующего provider.
- Parameterized provider подробных данных отличает ожидание первого snapshot, успешное отсутствие `AsyncData(null)` и типизированную ошибку чтения. Устранимая ошибка показывает локализованный retry, который продвигает `DetailObservationGeneration`, инвалидирует только текущую подписку данного идентификатора и создаёт новую `watchById`-подписку; уход последнего слушателя автоматически отменяет текущую подписку, а повторное открытие начинает новую вместо показа сохранённой ошибки.
...
Для маршрутов используются `auto_route` 11.1.0, `auto_route_generator` 10.6.0 и `MaterialApp.router`. `AppRouter` объявляется через `@AutoRouterConfig`, страницы — через `@RoutePage`, а единственным представлением внутренних переходов служат generated `PageRouteInfo` с обязательными типизированными аргументами; маршрут подробного просмотра принимает предметный `IntentionId` из любого охвата каталога. Route pop не переносит command result и не является его вторым каналом: Catalog ViewModel получает terminal outcomes только от coordinator независимо от того, закрыт ли дочерний экран до их завершения. Прикладной код не вызывает строковые `pushNamed`, `replaceNamed` или `navigateNamed` и не передаёт имена или path вручную. Router создаётся generated `keepAlive` provider внутри принадлежащего `AppRuntime` container; глобальный экземпляр router не вводится.
...
| `title` | `TEXT` | `NOT NULL`, непустое нормализованное значение без UTF-8 NUL byte |
| `title_search_key` | `TEXT` | `NOT NULL STORED GENERATED ALWAYS`, вычисляется только из `title`, не содержит UTF-8 NUL byte; не записывается adapter и не входит в предметную модель |
| `description` | `TEXT NULL` | `NULL` означает отсутствие; присутствующий текст не содержит UTF-8 NUL byte и хранится посимвольно |
...
Имя и сигнатура `doable_title_search_key(TEXT)` являются частью schema contract, а конкретная версия используемых Unicode-данных — сменной implementation dependency сборки. Совместимое обновление MAY продвинуть её без schema version bump и без обязательного массового пересчёта: нетронутая строка может сохранить ранее вычисленный ключ до следующей записи этой строки либо явно выбранного rebuild поисковой проекции и FTS. Этот редкий семантический дрейф сознательно принят только для восстанавливаемой поисковой проекции, которая не изменяет `title`, не участвует в предметной идентичности, уникальности, ссылочной целостности, авторизации или синхронизации. Будущее производное значение с любым из этих назначений не наследует этот компромисс и требует отдельного решения о стабильности, версии и миграции. Долговечность имени, сигнатуры, регистрации и trust boundary функции закрепляет ADR-0008.

Каждое каноническое текстовое поле и `title_search_key` получают defense-in-depth `CHECK`, который проверяет отсутствие нулевого byte через BLOB-представление значения, а не через `length()` или `quote()`: в UTF-8 единственным источником нулевого byte является `U+0000`. Это ограничение не пытается повторить подсчёт графем либо Unicode whitespace semantics средствами SQLite. Полный field-specific контракт проверяется общей Dart-границей до записи и повторно при чтении исходного storage-представления. Для catalog row закрытый mapper валидирует ограниченно выбранное описание и публикует только boolean `hasDescription`; `COUNT` и продолжения cursor не материализуют описания за пределами `pageSize + 1`. Значение, нарушающее общий Unicode-repertoire, нормализацию или длину поля, становится `IntentionCorruptionFailure`, а не успешной частичной моделью.

Adapter записывает только каноническое представление `IntentionId`, а при чтении проверяет формат, RFC variant и ненулевое значение до построения предметной модели. SQL-ограничения защищают обязательность сохранённых значений и отсутствие NUL, но не вводят `CHECK` взаимного порядка timestamps и не заменяют типизированное декодирование идентификатора и предметную валидацию расширенных графемных кластеров в repository: SQLite не является источником истины для формы UUID или пользовательски воспринимаемой длины Unicode-текста. Все запросы строятся типизированным API Drift или параметризованными variables; пользовательский текст не конкатенируется с SQL.
...
До предметной rehydration подробный и catalog read-path получают lossless storage-представление затронутой строки, в котором Drift ещё не преобразовал значения по типам generated data class. Реактивный detail query и ограниченный catalog query явно передают в закрытую mapping boundary необходимые исходные поля из `QueryRow.data`; внутренний generated `title_search_key` не проверяется как независимо сохранённое предметное значение. Boundary принимает `String` для `id`, `title` и присутствующего `description`, `null` для отсутствующего `description`, только целые `0` или `1` для readiness и archive state и только представимые целые микросекунды для обоих timestamps. `REAL`, `BLOB`, посторонний Dart-тип, `null` в обязательном поле, целое значение вне допустимого boolean-множества или значение вне поддерживаемого диапазона `DateTime` отклоняются до любого преобразования через generated `bool`/`int` mapping. Только после этой проверки adapter декодирует `IntentionId`, проверяет общий Unicode-repertoire, сверяет нормализацию и длину текста и строит предметное намерение либо summary; catalog mapper отбрасывает проверенное описание и переносит наружу только `hasDescription`. Любое фактическое нарушение завершает чтение через `IntentionCorruptionFailure` без частично подтверждённого snapshot или ложного count. Storage-specific представление остаётся закрытым внутри Drift adapter и не пересекает `IntentionRepository`.
...
Согласно ADR-0002 `QueryExecutor` остаётся внутренней низкоуровневой подстановкой модуля соединений; произвольный executor не является допустимым входом обычного object graph. Закрытый platform-neutral тип `ConfiguredLocalDatabaseConnection` владеет executor и служит единственным статическим доказательством того, что обязательный канонический pre-open setup привязан к пути открытия и будет выполнен для каждого создаваемого executor физического SQLite-соединения до первого использования зависимой schema. Capability не утверждает, что ленивое открытие или callback уже состоялись. `LocalDataReady` является доступным для тестов верхних слоёв runtime-result состоявшегося production bootstrap, но не другой capability и не неподделываемое type proof. Канонический production composition получает его только как возврат `LocalDataBootstrap` после фактического открытия и завершения migration; отдельный eager-open typestate не вводится. Только factories local connection module могут создать capability; `AppDatabase` принимает её, а `LocalDataBootstrap` — фабрику таких значений вместо фабрики raw `QueryExecutor`. Android production factory явно передаёт `getApplicationDocumentsDirectory` как `DriftNativeOptions.databaseDirectory` в `driftDatabase(name: 'doable', ...)`, получает файл `doable.sqlite` и выполняет SQLite в background isolate, а in-memory/file-backed и migration factories выбирают собственный native executor за той же типизированной границей.

Единый канонический connection setup регистрирует `doable_title_search_key` на каждом physical SQLite connection до разбора и использования schema. Для Doable-owned production, in-memory/file-backed и migration executors это обеспечивает `ConfiguredLocalDatabaseConnection`. Фактическая сторона полной schema verification также создаёт `AppDatabase` только из capability, но эталонное соединение создаёт сам `drift_dev` и потому capability на него не распространяется. Отдельный закрытый test/debug verifier helper является единственным обычным местом вызова низкоуровневого `validateDatabaseSchema`: он принимает фактическую configured `AppDatabase` и передаёт тот же канонический setup каждой создаваемой `drift_dev` reference connection через внешний `setup:` callback. Обычные verification harnesses вызывают helper и не передают произвольный setup напрямую.

Дополнительный fixture setup на обеих границах выполняется до канонической регистрации: последняя регистрация зарезервированного имени и arity всегда принадлежит Doable, поэтому обычная композиция не может неявно затенить schema-function. Capability-preserving decoration не предоставляется как generic extension: только закрытые adapters local-data module могут обернуть уже выбранный executor для конкретного tracing или fault injection и вернуть `ConfiguredLocalDatabaseConnection`. Caller передаёт такому adapter только hook без доступа к `QueryExecutor`; implementation безусловно делегирует `ensureOpen` тому же configured executor и не создаёт и не заменяет physical connection. Произвольный `QueryInterceptor` может участвовать только в явно названной raw test-only fixture ниже этой seam, которая не может создать capability или обычную `AppDatabase`. Любой decorator, владеющий созданием physical connection, сам проходит соответствующую factory или adapter boundary. Missing-setup и намеренный mapping drift проверяются только низкоуровневыми явно названными test fixtures вне обычного `AppDatabase`/bootstrap/verifier object graph; общий production-доступ к небезопасному constructor или произвольной замене зарезервированной функции не вводится. Универсальный registry либо DSL для schema-functions не добавляется, пока второй реальный function/collation contract не докажет необходимость такого обобщения.

Компонентная граница внутри Flutter application container выглядит так:

```text
+------------------------+   +-------------------------+   +----------------------+
| Android production     |   | In-memory / file-backed |   | Migration factories  |
| connection factory     |   | connection factories    |   |                      |
+-----------+------------+   +------------+------------+   +----------+-----------+
            \                         |                             /
             \                        v                            /
              +--------------------------------------------------+
              | fixture extension -> canonical setup is bound    |
              +-------------------------+------------------------+
                                        |
                                        v
                         +-----------------------------------+
                         ...
                         +-----------------+-----------------+
                              ...
                              +----------+----------+
                                         |
                          actual database|               canonical setup callback
                                         v                         |
                              +---------------------+              |
                              | Doable verifier     |--------------+
                              | helper              |              |
...
Raw missing-setup, alternate-mapping, and arbitrary-interceptor fixtures remain
a separate test-only branch and create no capability, ordinary AppDatabase,
or verifier helper.
```

- Factories владеют выбором платформы и физического executor; `AppDatabase` не получает Android paths или platform APIs.
- Opaque connection capability переносит через bootstrap, migrations и test harnesses неподделываемое доказательство привязанного pre-open setup для Doable-owned executors; `LocalDataReady` остаётся тестопригодным runtime-result канонического production bootstrap, а не второй capability.
- Doable verifier helper владеет внешним `drift_dev` callback и применяет тот же канонический setup к reference connection, не приписывая ей application capability.
- Только закрытые adapters local-data module сохраняют capability при tracing и fault injection: они безусловно делегируют open исходному configured executor и не дают caller подменить physical connection; generic `QueryInterceptor` такой гарантии не получает.
- Негативные и mapping-drift сценарии намеренно работают ниже обеих защищённых границ и потому не превращаются в альтернативный production constructor или verifier path.

`package:sqlite3` 3.5.2 помечает функцию `deterministic: true`; для каждой конкретной сборки и регистрации callback является неизменным total pure transform одного `TEXT`, не выполняет I/O, не читает окружающее состояние и не раскрывает данные, тогда как явно принятый межрелизный Unicode-дрейф не вносит изменяемого состояния внутрь connection. Необходимое для вызова из generated schema значение `directOnly: false` допустимо только при этих свойствах и внутреннем app-specific файле. Приложение не открывает импортированные или полученные извне SQLite-файлы. Если эта trust boundary меняется либо connection начинает использовать `trusted_schema = OFF`, до такого изменения требуется innocuous-capable registration или отказ от application-defined function в schema; молчаливое отключение setup недопустимо. ADR-0008 делает имя, сигнатуру, порядок регистрации и эту trust boundary долговечным ограничением будущих schema changes и connection adapters.

Зафиксированный `path_provider_android` разрешает каталог базы через `Context.getDir("flutter")`, то есть как `root/app_flutter/` в терминах Android Auto Backup. Оба набора Android backup rules исключают весь `root/app_flutter/` для cloud backup и device-to-device transfer; исключение каталога охватывает основной файл, соседние WAL/SHM и другие создаваемые SQLite служебные файлы, а SQLite temp directory находится в не включаемом в backup cache storage. Production locator и backup rules образуют один проверяемый Android host contract: тест разрешает выбранные connection name/directory в ожидаемые `domain/path` и падает при расхождении любого из них. Android-specific расположение и backup domain не пересекают `AppDatabase`, `IntentionRepository` или предметные interfaces. `ConfiguredLocalDatabaseConnection` не является новой общей storage interface поверх Drift: это узкая capability на существующей connection seam, которая скрывает прежний `QueryExecutor` и делает обязательный setup статическим инвариантом. Repository seam и возможность будущего adapter остаются без изменений.

`LocalDataBootstrap` является единственным lifetime-owner созданных им `AppDatabase` и содержащего executor `ConfiguredLocalDatabaseConnection`, а `AppRuntime` является единственным владельцем самого bootstrap и остального process-local object graph. В production возврат `LocalDataReady` является runtime-postcondition того, что ленивый executor успешно открыл physical connection, выполнил привязанный setup и завершил migration; это значение не является неподделываемой capability, а его публичная конструкция остаётся доступной тестам верхних слоёв. Сама connection capability подтверждает лишь корректность pre-open пути. После готового bootstrap `DriftIntentionRepository` получает заимствованный `AppDatabase`, не закрывает его и не добавляет lifecycle-операцию в storage-neutral `IntentionRepository`.

Первый вызов `LocalDataBootstrap.close()` синхронно переводит bootstrap в terminal closing state до любого `await` и сохраняет единственный Future закрытия. Повторный `close()` присоединяется к тому же завершению и не закрывает database ещё раз; `open()` после начала closing или после closed не может создать новое connection и считается нарушением внутреннего lifecycle contract. Явный retry устранимого bootstrap failure остаётся допустимым только пока runtime находится в running state и shutdown не начат. Это закрывает также гонку, при которой прежняя реализация могла обнулить ссылку на database перед завершением её `close()` и параллельно начать новый `open()`.

При управляемом освобождении `AppRuntime` сначала запрещает новую работу, прекращает provider consumers и process-local координацию, дожидается уже принятых repository operations, затем ожидает единственный `LocalDataBootstrap.close()`. `UncontrolledProviderScope` не уничтожает переданный ему container автоматически; `AppRuntime` явно вызывает синхронный `ProviderContainer.dispose()` в фазе quiesce и не разрешает создать следующий object graph над тем же файлом до завершения собственного `shutdown()`. File-backed и app-level tests воспроизводят этот порядок через production ownership, а не через fire-and-forget callback или отдельную test-only seam.
...
- unit tests общего `ExclusiveOperation` проверяют синхронный переход в `running`, отсутствие запуска или очереди второго действия, освобождение gate после success/failure и независимость экземпляров; coordinator tests с управляемыми completers подтверждают продолжение принятого Future после disposal editor/details, одну непереигрываемую публикацию terminal outcome нескольким одновременным data consumers, порядок completions по repository revisions, сохранение per-`IntentionId` gate при уходе и повторном открытии, независимость разных идентификаторов и отсутствие изменения подтверждённых данных при failure. Details tests устанавливают completion listener до первой read generation и управляемо переставляют прежний snapshot, `IntentionSaved` и результат новой подписки для update, readiness и archive/restore: success немедленно становится авторитетным, запоздалое значение прежней generation игнорируется, а post-success snapshot новой generation принимается. Delete fixture доставляет прежний non-null snapshot после `IntentionDeleted` и подтверждает, что route не возобновляется. Отдельные coordinator shutdown fixtures подтверждают синхронный запрет новых commands, ожидание всех уже принятых repository Futures после disposal provider graph, отсутствие обращения к уничтоженному `Ref` и завершение drain без ожидания presentation claims исчезнувшего UI. Repository tests доказывают одну process-local эпоху на экземпляр, последовательные revision только после фактических commits, неизменность revision при no-op/failure, revision-stamped page snapshots, несравнимость эпох после пересоздания и точные `before`/`after` membership-снимки, включая историческую поисковую проекцию предыдущей Unicode-сборки без раскрытия storage-полей. Тесты generated Riverpod ViewModels и providers используют отдельный `ProviderContainer` с overrides на fake repository и проверяют initial loading/data/empty/failure, накопление порций, single-flight следующей страницы, конец выдачи, debounce, отклонение устаревшего результата, три scope, четыре порядка, точный count первой страницы, сохранение count при продолжениях, подмену `CatalogPagingPolicy` и согласование typed completion после нескольких порций. После одной и нескольких загруженных порций отдельная continuation-матрица проверяет retry того же cursor только для `unavailable`, terminal inline-состояния `corruption` и `unexpected` без retry и явное восстановление `validation` первой страницей с `cursor: null`: прежние items/count/boundary остаются видимыми до успеха, ответ прежней generation отбрасывается, success целиком заменяет catalog snapshot, а failure сохраняет прежний префикс и использует initial-page классификацию. Параметризованная матрица `createdAt`/`updatedAt` × ascending/descending покрывает создание и изменение внутри и после boundary, неизменность cursor, немедленное обновление count по точному membership transition, сохранение visual anchor и последующую подгрузку без пропусков, повторов или преждевременно показанного намерения. Управляемые перестановки доставки покрывают старую страницу после нового completion, страницу, уже включающую commit до его completion, более новую страницу до предшествующих completions и смену query generation при выполняющейся операции; тесты подтверждают retry только затронутой устаревшей или unavailable-страницы и ограниченное состояние без множества всех обработанных tokens;
...
- connection-contract tests проводят Android production-like background, in-memory/file-backed, migration и фактическую schema-verification database через `ConfiguredLocalDatabaseConnection`, подтверждают привязку pre-open setup ко всем создаваемым executor physical connections и одинаковую прямую SQL-семантику зарезервированной функции. Отдельный bootstrap contract test подтверждает runtime-семантику `LocalDataReady`, не приписывая ему static capability. Тесты закрытых local-data-owned tracing и fault adapters доказывают, что каждый hook наблюдает или вводит отказ в заданной точке, но `ensureOpen` безусловно делегируется тому же configured executor и capability сохраняется. Отрицательное API evidence подтверждает, что generic `QueryInterceptor` не принимается capability-preserving interface и остаётся только raw test-only fixture. Отдельные verifier-helper tests доказывают, что только закрытая Doable seam передаёт тот же канонический setup каждой создаваемой `drift_dev` reference connection и что обычные harnesses не вызывают `validateDatabaseSchema(setup: ...)` напрямую. Fixture collision на обеих границах подтверждает последнюю каноническую регистрацию; raw test-only fixtures отдельно доказывают controlled failure зависимой схемы без setup и допустимый mapping drift двух сборок, не предоставляя обходной constructor для `AppDatabase` или альтернативный verifier path;
...
- app runtime lifecycle tests управляемыми completers проводят shutdown из ready и in-flight bootstrap, проверяют переход bootstrap в closing до первого ожидания, присоединение повторного `close()` к одному Future, невозможность нового `open()` и нового object graph над тем же файлом до полного закрытия, а также порядок coordinator drain → единственный database close. Внезапное завершение процесса отдельно остаётся crash-recovery границей и не подменяется ожиданием Flutter/Riverpod disposal callback;
...
- **[Keep-alive coordinator может удержать завершённую операцию, повторно опубликовать outcome или превратиться в неограниченный process-local журнал]** → Хранить только выполняющиеся entries, выдавать каждому запуску непрозрачный presentation token, публиковать один terminal completion в порядке repository revisions и удалять entry в `finally`; Catalog ViewModel согласует данные по revision, а не по вечному множеству tokens, и хранит только конечные in-flight completions и не более одной pending page. Delayed-completion tests покрывают disposal, повторное открытие, перестановки page/completion и success/failure. Новый каталог всегда начинает с repository snapshot, поэтому coordinator не хранит replay log и не заменяет будущую sync seam.
- **[Поздняя страница может вернуть состояние до уже применённого commit, а страница после commit — привести к повторному изменению count или membership]** → Сериализовать snapshot-чтения страниц и изменяющие commits одним repository sequencer, штамповать оба результата process-local revision, возвращать точные непрозрачные `before`/`after` и применять правила older/same/newer. Старую страницу повторять только от прежнего cursor; страницу впереди каталога удерживать до предшествующих completions. Цена — дополнительное ожидание записи за уже начатым ограниченным page read; текущие `pageSize <= 100` и 100-миллисекундный CI regression budget делают его приемлемым, а наблюдаемое превышение требует отдельного решения с измерениями.
- **[Запоздалый detail snapshot прежней подписки может отменить показ уже подтверждённого success или вернуть удалённое намерение]** → Устанавливать listener coordinator completions до первой detail read generation, считать matching `IntentionSaved` единственным авторитетным terminal data result, до нового `watchById` продвигать приватную generation и проверять её в каждом обработчике вместо доверия одной отмене подписки. `IntentionDeleted` продвигает generation до завершения route. Управляемые tests переставляют старый snapshot, success и новый snapshot для изменения данных и состояний, а после delete доставляют прежнее non-null значение.
- **[Automatic retry Riverpod может скрыть первый failure и нарушить явную модель повторной попытки]** → Отключить глобальный retry во владеющем `ProviderContainer` и повторять только целевой provider по явному действию пользователя.
- **[Синхронные Flutter/Riverpod disposal callbacks не могут подтвердить завершение асинхронного закрытия базы]** → Владеть bootstrap и container через process-local `AppRuntime`, использовать `UncontrolledProviderScope` только для доступа UI, разделить shutdown на синхронный quiesce и ожидаемый drain/close, сделать closing bootstrap terminal и не создавать следующий object graph над тем же файлом до завершения `shutdown()`.
...
- **[Application-defined search-key function вызывается из schema, а новый connection path или fixture может пропустить либо затенить её setup]** → Разрешить schema-вызов только для total pure function одного `TEXT`, которая не выполняет I/O, не читает внешнее состояние и не раскрывает данные; открывать только внутренний app-specific SQLite-файл без импорта. Передавать в `AppDatabase` и bootstrap только непрозрачную `ConfiguredLocalDatabaseConnection`, доказывающую привязанный ко всем executor connections pre-open setup. `LocalDataReady` сохранять как тестопригодный runtime-result успешного production bootstrap, но не считать его статическим доказательством состоявшегося открытия и не вводить ради этого eager-open typestate. Для connections, создаваемых внешним инструментом, вводить узкий owner-specific adapter; текущий verifier helper единолично передаёт канонический setup в `drift_dev` callback. Сохранять capability при tracing и fault injection только за закрытыми adapters local-data module, которые безусловно делегируют open тому же configured executor и не дают hook доступ к `QueryExecutor`; generic interceptor оставлять только в raw test-only fixture. Выполнять fixture setup до канонического и оставлять missing-setup и alternate mapping в явно отделённых raw test fixtures. При появлении импорта чужих баз, `trusted_schema = OFF` или более широкой модели угроз сначала перейти на innocuous-capable registration либо убрать application-defined function из schema.
...
- **[Dart `String` допускает непарный UTF-16, а SQLite и FTS неодинаково трактуют встроенный NUL]** → До любой нормализации и UTF-8/SQLite-границы проверять code units общей операцией из ADR-0007, отклонять непарные surrogates и `U+0000` типизированно, дублировать NUL-инвариант schema `CHECK` и покрывать domain, command, catalog, raw-write и FTS boundaries одним adversarial corpus. Не расширять этот запрет на остальные корректные scalar values и не добавлять скрытую Unicode-нормализацию.
...
3. Ввести process-local `AppRuntime` с явно принадлежащими ему `LocalDataBootstrap` и `ProviderContainer`, передать container UI через `UncontrolledProviderScope`, собрать generated providers, bootstrap shell, локализацию и routing, сохранив доступный loading/error до готовности базы и ожидаемый двухфазный shutdown до повторного использования файла.
4. Явно связать Android production connection `root/app_flutter/doable.sqlite` с исключением всего `root/app_flutter/` в обоих наборах backup rules и их contract test, закрыть Doable-owned production/test/migration executors за `ConfiguredLocalDatabaseConnection` с привязанным pre-open setup, оставить tracing и fault injection за закрытыми local-data-owned adapters без generic `QueryInterceptor`, а `drift_dev` reference connection — за отдельным test/debug verifier helper, который один передаёт канонический callback; зарегистрировать на каждом physical connection чистую deterministic `doable_title_search_key`, затем создать `AppDatabase` schema version 1 с UUID v7, временными метками намерения, единственным записываемым `title`, NUL-защитой канонического текста и поисковой проекции, вычисляемым `STORED` search key, одноколоночным FTS5 trigram-индексом этой проекции, единой local search seam буквальной подстроки, triggers и index-aware `integrity-check` с отрицательной fixture, атомарными первичным созданием и migration harness, полным семантическим сравнением schema objects, ограниченным production bootstrap без `drift_dev`, классификацией `CORRUPT`/`NOTADB`, initial schema snapshot, проверкой несовместимой более новой версии и in-memory executor.
5. Реализовать глубокий `IntentionRepository` с ограниченным `getCatalogPage`, точным count, keyset cursor, process-local revision protocol, точными `before`/`after` catalog mutations, предметными моделями и failures, затем generated keep-alive `IntentionCommandCoordinator`, auto-dispose Riverpod ViewModels, `CatalogPagingPolicy`, immutable operation states и Views.
...
ADR-0002 закрепляет общую типизированную connection boundary и скрывает raw `QueryExecutor` внутри модуля соединений. ADR-0003 фиксирует type-safe границу идентичности и оставляет версию UUID сменной политикой production generator. ADR-0005 заменяет ADR-0001 внутри change: граница глубокого модуля сохранена, а наблюдение всего каталога заменено ограниченными страничными snapshot-запросами. ADR-0007 задаёт общий Unicode-repertoire пользовательского текста графа, а ADR-0008 — долговечный контракт application-defined SQLite-функций, используемых схемой. Действующими для этого change являются ADR-0002–ADR-0008 со статусом `proposed`; ADR-0001 остаётся историческим контекстом. Окончательное принятие ADR выполняется при архивации.
