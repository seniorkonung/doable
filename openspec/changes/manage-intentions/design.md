...
- Приложение запускается внутри одного корневого `ProviderScope`. Database, repository, diagnostics, `IntentionCommandCoordinator` и router публикуются через generated `keepAlive` providers, а их внешние ресурсы освобождаются через `ref.onDispose`.
...
- Уход с editor или details всегда доступен и не вызывает cancellation принятого command. Если экран остаётся открытым, его ViewModel наблюдает тот же accepted `Future`, показывает terminal state и потребляет presentation claim; явный retry получает новый token и не возвращает уже потреблённый failure в каталог. После disposal coordinator завершает работу без обращения к уничтоженному `Ref`, а непотреблённый outcome передаёт catalog fallback. Повторное открытие details читает running state coordinator по `IntentionId`. Завершение корневого `ProviderScope` прекращает только in-process координацию и не обещает продолжение Future после остановки процесса; SQLite-транзакция и следующий bootstrap обеспечивают уже заданную атомарность и восстановление подтверждённого состояния. Экспериментальные Riverpod Mutations, offline persistence, durable event log и sync abstraction не используются.
