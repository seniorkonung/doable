...
- repository получает функцию текущего UTC wall-clock time через constructor injection, чтобы создание, обновление и граничные случаи одинаковых временных меток были детерминированно проверяемы без отдельного глобального clock service;
