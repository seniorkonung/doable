...
Cursor остаётся storage-neutral opaque interface без публичных полей и фабрик: caller только возвращает полученный экземпляр repository и не строит его из SQLite row, offset или SQL-выражения. Конкретный adapter владеет реализацией cursor, связывает нормализованные параметры запроса с граничной парой выбранной временной метки и `IntentionId` и проверяет полученный cursor до обращения к storage. Поэтому подтверждённое создание, изменение или удаление при неизменных параметрах запроса не инвалидирует сохранённую границу; прежний cursor отбрасывается только при изменении scope, фильтра или порядка. Cursor другого adapter, с другой нормализованной комбинацией параметров либо структурно недопустимое значение возвращает validation failure.

`IntentionCatalogPage` является sealed-контрактом и всегда содержит не больше запрошенного ограниченного размера и cursor следующей порции либо `null` после конца выдачи. Запрос без cursor возвращает только `IntentionCatalogFirstPage` с точным количеством всех совпадений до разбиения на порции; её release-safe constructor не допускает отрицательный count или count меньше числа возвращённых summaries. Запрос с cursor возвращает только `IntentionCatalogContinuationPage` без повторного count. Поэтому отсутствие count у продолжения не представляется nullable-полем, а caller исчерпывающе различает начальный snapshot и его продолжение. `IntentionSummary` несёт только необходимые каталогу идентификатор, название, наличие описания, готовность, архивное состояние и timestamps; полный текст описания получает только `watchById`. Предметная модель и summary не допускают `updatedAt` раньше `createdAt`, а adapter преобразует такое нарушение при rehydration в corruption failure. Это ограничивает I/O и память независимо от размера описаний. Невалидный или не соответствующий запросу cursor, размер порции вне диапазона 1–100 либо непустой нормализованный фильтр длиннее 255 расширенных графемных кластеров возвращают стабильную validation failure до обращения к storage adapter, а не выполняют чтение каталога.
...
Для локального хранения выбирается Drift поверх SQLite через `drift` и `drift_flutter`; генерация и runtime schema verifier используют `drift_dev`, а orchestration генерации — `build_runner`. Поскольку verifier вызывается production-кодом, `drift_dev` становится обычной прямой dependency приложения, а не только dev dependency; tree shaking и фактический release-size проверяются измерением APK. На момент design проверены совместимые с Dart 3.13.1 версии `drift` 2.34.3 и `drift_flutter` 0.3.1. Drift выбран за типизированные запросы, реактивные streams, транзакции и инструменты миграций; эти возможности описаны в [официальной документации Drift](https://drift.simonbinder.eu/).
...
Допустимый непустой фильтр, чей готовый search key содержит не меньше трёх кодовых точек Unicode, проходит через единственный storage helper построения буквального FTS-выражения. Helper удваивает каждый символ `"` внутри search key и затем заключает весь результат в одну пару двойных кавычек согласно [синтаксису строк FTS5](https://www.sqlite.org/fts5.html#fts5_strings); только после этого готовая phrase передаётся SQL-параметром `MATCH`. SQL parameterization защищает границу SQL, а отдельное FTS-экранирование не позволяет кавычкам, `OR`, `AND`, `NOT`, `NEAR`, скобкам, `*`, дефисам или пробелам пользовательского текста стать операторами поисковой грамматики. Production repository обязан переиспользовать этот helper и не строит `MATCH`-выражение самостоятельно.

Поскольку trigram не индексирует подстроки короче трёх кодовых точек, допустимый search key из одной или двух кодовых точек использует параметризованный `instr(title_search_key, query_search_key)` по основной таблице. Фильтр длиннее 255 расширенных графемных кластеров отклоняется до построения search key и FTS phrase, открытия read transaction, `COUNT` или чтения строк. Обе ветви допустимого поиска проверяются одинаковыми fixtures, а FTS-ветвь дополнительно покрывает кавычки, операторы, скобки, `*`, дефисы, внутренние пробелы и Unicode, чтобы наблюдаемая семантика буквальной подстроки не зависела от выбранного storage path.

`getCatalogPage` применяет scope и фильтр до count и порядка. Active/archive используют явный predicate `is_archived`, all не добавляет его. Для каждого поля времени создаются индексы, поддерживающие `ORDER BY <timestamp> ASC|DESC, id ASC`; cursor predicate сравнивает выбранную временную метку, а при её равенстве — `id`. Repository запрашивает `pageSize + 1` summaries, возвращает не больше `pageSize` и строит следующий cursor только при наличии дополнительной строки. Offset не используется, поэтому стоимость прохода не растёт пропорционально номеру условной страницы. Для запроса без cursor точный `COUNT` и строки первой порции выполняются внутри одной read transaction и видят один SQLite snapshot. Для запроса с cursor выполняется только чтение строк продолжения: неизменный count уже принадлежит состоянию Catalog ViewModel, а подтверждённые commands корректируют его по переходу принадлежности текущему scope/filter. Поэтому полный последовательный проход одного неизменного query выполняет ровно один точный `COUNT` независимо от числа порций.
...
После проверки версии и завершения применимой миграции production `beforeOpen` вызывает `validateDatabaseSchema(options: const ValidationOptions(validateDropped: true))`. Drift читает `CREATE`-определения фактических объектов из `sqlite_schema`, создаёт вторую пустую in-memory database с эталонной текущей схемой через `Migrator.createAll()` и семантически сравнивает оба набора. Проверка охватывает наличие и вид schema objects, колонки и их ограничения, определения обычных и virtual tables вместе с FTS5-конфигурацией, тела triggers, а также поля, порядок и predicates indexes; `validateDropped: true` дополнительно отклоняет лишние пользовательские schema objects. Совпадение одних имён недостаточно.

Эта проверка ограничена размером схемы: она не читает пользовательские строки, не выполняет FTS `integrity-check` и не сканирует весь поисковый индекс при каждом старте. Полный index-aware audit остаётся отдельным helper и вызывается внутри атомарного первичного создания, каждой миграции, затрагивающей FTS, а также из явно выбранного будущего recovery/maintenance flow; обычный bootstrap не выбирает такой flow неявно. Instrumented file-backed test проверяет отсутствие пользовательских reads и команды `integrity-check` при повторном открытии текущей схемы независимо от объёма данных.
...
Ограниченная проверка текущей схемы создаёт `CorruptLocalDataSchemaException` только тогда, когда успешно прочитанный marker либо `SchemaMismatch` от семантического сравнения доказывает несовместимое состояние. Ошибки выполнения самого verifier, включая невозможность прочитать schema metadata или создать эталонную in-memory database, сохраняют исходную типизированную причину и покидают migration boundary без преобразования в corruption.

На boundary открытия bootstrap последовательно раскрывает известные транспортные обёртки Drift, включая `DriftRemoteException.remoteCause` production background executor, и классифицирует найденное `SqliteException` по первичному `resultCode`, а не по локализованному сообщению или полному exception. `SQLITE_CORRUPT` и `SQLITE_NOTADB`, включая extended variants с тем же первичным кодом, преобразуются в non-retryable `LocalDataCorruption`. Явный allowlist `SQLITE_BUSY`, `SQLITE_LOCKED`, `SQLITE_CANTOPEN` и `SQLITE_IOERR` преобразуется в `LocalDataRetryableFailure`, поскольку повтор способен восстановить работу после блокировки или временной недоступности файловой системы.
...
Repository преобразует ожидаемые validation, not-found, conflict, unavailable и corruption outcomes в закрытый набор failures. Bootstrap отдельно различает retryable opening/migration failure, corruption, non-retryable `incompatibleSchema` и non-retryable `unexpected`; несовместимость содержит только ожидаемую и обнаруженную версии схемы и требует обновления приложения, а unexpected не утверждает, что файл повреждён или повтор восстановит работу. Неожиданные exceptions перехватываются на ближайшей ответственной seam, передаются в diagnostics и превращаются в общий безопасный failure; stack trace и внутренние детали пользователю не показываются. Типизированный `Result` следует документированному Flutter подходу к [обработке ошибок между слоями](https://docs.flutter.dev/app-architecture/design-patterns/result).
...
- runtime schema-validation tests сохраняют текущий marker и ожидаемые имена, но по отдельности изменяют колонки и constraints, FTS5-конфигурацию, тело trigger, поля или predicate index и добавляют лишний schema object; каждый `SchemaMismatch` даёт corruption без feature access или изменения файла, а instrumented large fixture доказывает отсутствие пользовательских reads и полного FTS audit. Отдельные повторяемые измерения на одном toolchain фиксируют длительность bootstrap пустой и представительной file-backed базы, а сопоставимые release APK builds — абсолютное и относительное изменение размера после включения verifier в production;
- bootstrap classification tests вводят `CORRUPT`, `NOTADB`, allowlisted временные коды и неизвестную причину непосредственно во время ограниченного чтения marker/schema metadata, а production-подобный background executor подтверждает раскрытие `DriftRemoteException`, одинаковые outcomes, закрытие неготового executor и безопасные diagnostics по ту сторону isolate boundary;
- file-backed repository integration test создаёт представительные активные и архивированные намерения, закрывает `AppDatabase` и отбрасывает первый `IntentionRepository`, затем создаёт новый persistence object graph на том же SQLite-файле и проверяет прежние идентификаторы, текст, готовность к действию, архивное состояние, timestamps, фильтр и точный count через публичную seam; отдельная повреждённая fixture с некорректным либо nil идентификатором подтверждает corruption failure без частично построенной предметной модели, а корректные UUID v4 и UUID v7 восстанавливаются одинаково. После повторного открытия storage-level harness отдельно вызывает явный FTS `integrity-check` с `rank = 1`, тогда как instrumented bootstrap доказывает отсутствие этого полного audit в обычном `beforeOpen`. Отдельные file-backed tests проверяют rollback и повтор первичного создания и upgrade, а fixtures `NOTADB`/`CORRUPT` — non-retryable классификацию с закрытием executor.
...
- **[Полный FTS audit на каждом запуске сделал бы стоимость bootstrap линейной по объёму индекса]** → Обычный `beforeOpen` семантически сравнивает только schema metadata с пустым эталоном, полный audit выполняется только в атомарных schema changes или явно выбранном recovery/maintenance flow, а SQL-trace test запрещает пользовательские reads и `integrity-check` при повторном открытии текущей схемы.
- **[Production schema verifier увеличивает release-код и добавляет создание второй пустой базы при bootstrap]** → Сознательно принять эту фиксированную цену ради обнаружения несовместимой структуры невосстанавливаемых локальных данных, оставить проверку независимой от числа записей и до завершения Phase 5 измерить длительность bootstrap и изменение размера release APK на одном toolchain. Drift рекомендует runtime-проверку преимущественно для debug именно из-за объёма дополнительного кода; существенно непредвиденная регрессия требует отдельного человеческого решения, а не неявного ослабления проверки.
...
- **[Новая фактически временная причина может отсутствовать в retryable allowlist и получить unexpected outcome]** → Считать retryable только доказанные машинными кодами причины, проверять классификацию на in-process и production-подобном background executor и расширять allowlist отдельным подтверждённым решением вместо catch-all retry.
...
4. Явно связать Android production connection `root/app_flutter/doable.sqlite` с исключением всего `root/app_flutter/` в обоих наборах backup rules и их contract test, затем создать `AppDatabase` schema version 1 с UUID v7, временными метками намерения, внутренним поисковым ключом, FTS5 trigram-индексом, единым helper буквальной phrase, triggers и index-aware `integrity-check` с отрицательной fixture, атомарными первичным созданием и migration harness, production schema verifier с полным семантическим сравнением schema objects без сканирования данных, классификацией `CORRUPT`/`NOTADB`, initial schema snapshot, проверкой несовместимой более новой версии и in-memory executor; до перехода к repository зафиксировать стоимость verifier для bootstrap и release APK.
