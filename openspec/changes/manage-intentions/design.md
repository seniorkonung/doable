...
3. Явно связать Android production connection `root/app_flutter/doable.sqlite` с исключением всего `root/app_flutter/` в обоих наборах backup rules и их contract test, затем создать `AppDatabase` schema version 1 с временными метками намерения, внутренним поисковым ключом, FTS5 trigram-индексом, его triggers и index-aware `integrity-check` с отрицательной fixture, initial schema snapshot, атомарным migration harness, проверкой несовместимой более новой версии и in-memory executor.
