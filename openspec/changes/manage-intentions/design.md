...
Обычный `beforeOpen` уже текущей версии выполняет только ограниченную проверку совместимости: читает marker версии и конечный набор обязательных имён schema objects, затем включает `foreign_keys`. Он не запускает FTS `integrity-check` и не сканирует пользовательские строки или весь индекс при каждом старте. Полный index-aware audit остаётся отдельным helper и вызывается внутри атомарного первичного создания, каждой миграции, затрагивающей FTS, а также из явно выбранного будущего recovery/maintenance flow; обычный bootstrap не выбирает такой flow неявно. Instrumented file-backed test проверяет отсутствие команды `integrity-check` при повторном открытии текущей схемы независимо от объёма данных.
...
На boundary открытия bootstrap классифицирует типизированное `SqliteException` по первичному `resultCode`, а не по локализованному сообщению или полному exception: `SQLITE_CORRUPT` и `SQLITE_NOTADB`, включая их extended variants с тем же первичным кодом, преобразуются в non-retryable `LocalDataCorruption`. `SQLITE_BUSY`, `SQLITE_LOCKED`, `SQLITE_CANTOPEN`, `SQLITE_IOERR` и остальные нераспознанные opening failures остаются `LocalDataRetryableFailure`, чтобы временная блокировка или недоступность файловой системы не объявлялась повреждением. В обоих случаях неготовый executor закрывается, а diagnostics получает только allowlisted failure code без SQL, параметров и текста исключения. File-backed fixtures отдельно покрывают произвольный non-database файл и повреждение header/schema, приводящие к `NOTADB` либо `CORRUPT`.
...
- **[Полный FTS audit на каждом запуске сделал бы стоимость bootstrap линейной по объёму индекса]** → Обычный `beforeOpen` ограничивается marker и конечным набором schema objects, полный audit выполняется только в атомарных schema changes или явно выбранном recovery/maintenance flow, а SQL-trace test запрещает `integrity-check` при повторном открытии текущей схемы.
