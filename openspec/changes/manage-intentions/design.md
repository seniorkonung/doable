...
Doable сейчас является стандартным Flutter-каркасом: один `MaterialApp` с экраном-заглушкой, без прикладных зависимостей и постоянного хранилища. Зафиксированный стек — Flutter 3.47.1 stable и Dart 3.13.1. Каталог `docs/adr/` отсутствует, поэтому действующих или заменённых ADR, ограничивающих этот design, нет.

`manage-intentions` — первая законченная пользовательская вертикаль. Она должна провести правила намерения через Android-интерфейс, прикладное состояние и локальную долговечность данных, но не должна преждевременно моделировать долговременные связи, дневной выбор или облачную синхронизацию. Создаваемая основа при этом должна допускать последующее расширение SQLite-схемы отношениями, путями и тегами без замены способа хранения.
...
- сделать подтверждённое состояние SQLite единственным источником истины и не показывать незафиксированные записи как сохранённые;
...
- поддержать русскую и английскую локализацию, TalkBack, системное масштабирование текста и безопасную диагностику с первого release capability.

**Не-цели:**

- моделировать долговременные связи, связь «я сегодня», пути выбора, теги, поиск или выполнение;
- добавлять учётную запись, облачную синхронизацию, сетевой API, аналитику или внешний crash-reporting;
...
Система состоит из одного развёртываемого контейнера — Flutter-приложения для Android — и принадлежащего ему SQLite-файла во внутреннем app-specific storage. Сетевых контейнеров и внешних сервисов в этом change нет.

```text
+----------------+        использует        +----------------------------------+
|   Пользователь | -----------------------> | Doable для Android               |
+----------------+                          | Flutter-приложение                |
                                            |                                  |
+----------------+  локаль, TalkBack,       |  +----------------------------+  |
|   Android OS   |  font scale, lifecycle   |  | UI и управление намерениями |  |
+----------------+ -----------------------> |  +----------------------------+  |
                                            |               |                  |
                                            |               | читает / пишет    |
                                            |               v                  |
...
Внешний deep link остаётся недоверенной строковой границей независимо от библиотеки: generated маршрут подробного просмотра принимает обязательный строковый path-параметр, после чего routing adapter явно разбирает его как `IntentionId`. Некорректный параметр и отсутствие сущности дают отдельные локализованные состояния, а не exception. AutoRoute сохраняет пространство для nested stack/tab routers, guards, declarative navigation и custom transitions в будущей навигации по графу, но текущий change не вводит эти конструкции без пользовательской необходимости. Императивный `Navigator` остаётся только для pageless подтверждений.
...
### 4. Drift и SQLite как постоянный источник истины
...
Используется SDK-пакет `flutter_localizations` и `gen_l10n` с ARB-файлами `app_en.arb` и `app_ru.arb`; английский файл является template. `supportedLocales` содержит только `en` и `ru`, а `localeListResolutionCallback` явно реализует правило: русский language code выбирает `ru`, английский — `en`, всё остальное — `en`. Это устраняет зависимость fallback от случайного порядка списка; механизм соответствует [документации Flutter по internationalization](https://docs.flutter.dev/ui/internationalization).
...
Views преимущественно используют стандартные Material controls с корректными roles и focus order. Пользовательские составные элементы получают `Semantics` с локализованными label/value/state; готовность и архивное состояние дублируются текстом или semantics, а не кодируются только цветом. Ошибки формы связываются с полем и объявляются TalkBack; результат destructive/изменяющей операции доступно сообщается после завершения. Layout проверяется при Android nonlinear text scale до 200%, без фиксированной высоты для пользовательского текста. Flutter рекомендует тестирование TalkBack, минимальные targets 48×48 и работоспособность при большом масштабе в [accessibility checklist](https://docs.flutter.dev/ui/accessibility).

### 7. Приватность, trust boundaries и hardening

Активы — пользовательские названия и описания, идентичность намерений и целостность архивного состояния. Trust boundaries проходят через текстовые поля, route parameter с идентификатором, SQLite-файл и входную схему миграции. Аутентификации и сетевой boundary нет: приложение обслуживает одного локального пользователя Android-профиля.

Применяются следующие меры:

- SQLite-файл, WAL и временные данные размещаются только во внутреннем app-specific storage; Android изолирует это хранилище от других приложений, что соответствует [Android security guidance](https://developer.android.com/privacy-and-security/security-best-practices#store-data-safely);
- release manifest не получает `INTERNET` или разрешения внешнего хранилища в рамках change;
- база явно исключается из cloud backup и device-to-device transfer правилами для Android 12+ `data-extraction-rules` и Android 11- `full-backup-content`; backup по умолчанию не должен незаметно выносить личный граф за локальную границу;
- route parameter, название и описание, включая границы в 255 и 4096 расширенных графемных кластеров, валидируются до использования; generated/параметризованные Drift queries исключают SQL injection;
