...
- [ ] 6.20b Сделать обязательный SQLite setup типобезопасной границей создания `AppDatabase`
  - **Критерии приёмки:**
    ...
    - Production background, in-memory/file-backed, migration и фактическая сторона schema verification получают `AppDatabase` через capability. Сохранить её при tracing и fault injection могут только закрытые adapters local-data module: caller передаёт только наблюдателя или политику отказа без доступа к `QueryExecutor`, а implementation безусловно делегирует `ensureOpen` исходному configured executor и не может создать или заменить physical connection. Generic `QueryInterceptor` не принимается таким interface и не может вернуть capability; любой decorator, создающий physical connection, сам проходит factory boundary.
  ...
  - **Проверка:**
    - Выполнить `flutter test test/data/local/database_connection_test.dart test/data/local/bootstrap/local_data_bootstrap_test.dart test/data/local/app_database_schema_test.dart`: bootstrap tests подтверждают runtime-семантику `LocalDataReady`, а production-like, in-memory/file-backed, composed-setup, tracing и fault-injection fixtures — делегирование open тому же configured executor и сохранение capability.
    - Выполнить `flutter analyze` и статическую проверку публичной и тестовой connection surface: обычный constructor `AppDatabase` не принимает raw `QueryExecutor`, capability-preserving interface не принимает generic `QueryInterceptor`, а сырой adversarial interceptor с переопределённым `ensureOpen` не может получить capability или обычную `AppDatabase`.
    - Выполнить `openspec validate manage-intentions --type change --strict --no-interactive`.
  - **Зависимости:** 6.20a.
...
- [ ] 6.20c Закрыть обходы SQLite setup в Doable-owned и `drift_dev` verification paths
  - **Критерии приёмки:**
    - Все поддерживаемые repository, bootstrap и migration harnesses, а также фактическая сторона schema verification создают `AppDatabase` только из `ConfiguredLocalDatabaseConnection`; прямые вызовы `AppDatabase(NativeDatabase...)` отсутствуют, а общий test harness и fault injection используют только узкие local-data-owned hooks и сохраняют единый lifetime/close contract. Произвольные interceptors остаются только в отделённых raw negative fixtures. Этот критерий supersedes историческую формулировку выполненной задачи 6.8 только в части прямого владения `QueryExecutor`: `LocalDataBootstrap` по-прежнему единожды закрывает созданную им `AppDatabase`, но executor скрыт внутри `ConfiguredLocalDatabaseConnection` и не принадлежит отдельному repository либо harness contract.
  ...
  - **Проверка:**
    ...
    - Выполнить `openspec validate manage-intentions --type change --strict --no-interactive`.
  - **Зависимости:** 6.20b.
  - **Вероятно затронутые файлы:** `test/support/local_database_harness.dart`, отдельный Doable schema-verifier helper в `test/support`, repository catalog/command/watch/fault tests, connection/bootstrap tests, migration/schema-verification tests, test-only raw SQLite fixtures.
  - **Оценка:** M (до 5 файлов или групп артефактов).
