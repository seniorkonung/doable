...
| [0009 — Объединить управление личным графом и согласование его снимков](0009-unify-personal-graph-module-and-revision.md) | Расширить единственную границу до `PersonalGraphRepository`, согласовывать ограниченные снимки общей ревизией и атомарно обновлять сводку с открытой группой; предложить замену ADR-0005. | proposed | — |
...
| [0012 — Централизовать предъявление результатов операций графа](0012-centralize-graph-operation-result-presentation.md) | Подтверждать и освобождать claims через общие владельцы точных поверхностей инлайн-сообщения и `SnackBar`. | proposed | — |

ADR-0009–ADR-0012 принадлежат активному изменению `manage-long-term-relations` и остаются `proposed` до его архивации. Предложенная ADR-0009 замена не меняет статус принятого ADR-0005; целевой контракт этого изменения учитывает её явно.
